...
async function init(newToken = false) {
  ...
  authProvider.addUser(
    (await getTokenInfo(tokenData.accessToken, config.twitch_auth.clientId))
      .userId,
    tokenData,
    ["chat", "channel"],
  );
  ...
  chatClient = new ChatClient({
    authProvider,
    channels: config.twitch_auth.channels,
  });
  ...
  // EventSub
  // Channel points
  const targetChannel = await apiClient.users.getUserByName(
    config.twitch_auth.channels[0],
  );
  ...
  for (var key in evtSubList) {
    ...
    evtSub["on" + evtSubList[key].apiName](
      targetChannel.id,
      ...(evtSubList[key].modParam
        ? [targetChannel.id, targetListener]
        : [targetListener]),
    );
  }
...
const refreshServer = createServer((req, res) => {
  ...
  if (
    new URLSearchParams(req.url.split("?")[1]).get("otp") != otpInstance.totp()
  ) {
    res.statusCode = 404;
    res.end();
    return;
