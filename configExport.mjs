...
const readJson = async (str) =>
  JSON.parse((await await readFile(str)).toString());

const twitchConfig = await readJson(
  import.meta.dirname + "/configs/twitch.json",
);
Object.freeze(twitchConfig);

const listenerConfig = await readJson(
  import.meta.dirname + "/configs/listeners.json",
);
