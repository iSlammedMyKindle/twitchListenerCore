...
const connectionClose = (ws) => {
  ...
  for (const i in existingConnections)
    twitchEmitter.off(i, existingConnections[i]);
...
if (config.ws.use_wss) {
  ...
  const server = createServer({
    cert: await readFile(
      path.resolve(import.meta.dirname + "/" + config.https.certpath),
    ),
    key: await readFile(
      path.resolve(import.meta.dirname + "/" + config.https.keypath),
    ),
